...
import com.example.pojokshoesapk.databinding.ActivityFormDeepcleanBinding
...
class ActivityFormDeepClean : AppCompatActivity(), FormPesanAdapter.OnQuantityChangeListener {
    private lateinit var binding: ActivityFormDeepcleanBinding
    private val TAG = "ActivityFormDeepClean"
    private lateinit var sessionManager: SessionManager

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityFormDeepcleanBinding.inflate(layoutInflater)
    ...
    private fun setupViews() {
        binding.icbackformdeepclean.setOnClickListener {
            finish()
        }
        ...
        binding.btnAddToCart.setOnClickListener {
            ...
            when {
                selectedServices.isEmpty() -> {
                    showError("Pilih layanan terlebih dahulu!")
                }
                !validateServices(selectedServices) -> {
                    showError("Jumlah sepatu tidak valid!")
                }
                else -> {
                    postKeranjang(selectedServices)
                }
            }
        }
    }
    ...
    private fun validateServices(services: List<DataListLayanan>): Boolean {
        return services.all { it.quantity >= 0 } &&
                services.any { it.quantity > 0 }
    }
        ...
        RetrofitClient.apiService.getLayananList().enqueue(object : Callback<List<DataListLayanan>> {
            override fun onResponse(call: Call<List<DataListLayanan>>, response: Response<List<DataListLayanan>>) {
                if (response.isSuccessful) {
                    ...
                    val deepCleanList = layananList.filter {
                        it.layanan_nama.contains("Deep Clean", ignoreCase = true)
                    }
                    deepCleanList.forEach { it.quantity = 0 }
                    if (deepCleanList.isNotEmpty()) {
                        setupListView(deepCleanList)
                    } else {
                        showError("Tidak ada layanan Deep Clean tersedia")
                    }
                } else {
                    handleErrorResponse(response, "Gagal memuat data layanan")
                }
            }
                ...
                override fun onResponse(
                    ...
                    if (response.isSuccessful) {
                        ...
                        if (detailResponse?.success == true) {
                            ...
                            // Panggil API untuk update total harga keranjang
    ...
    // Fungsi untuk memanggil API update total harga keranjang
                ...
                override fun onResponse(call: Call<List<KeranjangItem>>, response: Response<List<KeranjangItem>>) {
                    if (response.isSuccessful) {
                        ...
                        if (activeKeranjang != null) {
                            ...
                            Log.d("Keranjang", "onResponse: Buat keranjang menggunakan keranjang aktif")
                            onResult(activeKeranjang.keranjang_id)
                        } else {
                            // Jika tidak ada keranjang aktif, buat keranjang baru
                            Log.d("Keranjang", "onResponse: Buat keranjang baru karena tidak ada yang aktif")
                            createKeranjang(pelangganId, totalHarga, onResult, onError)
                        }
                    } else {
//                        onError("Gagal memuat keranjang: ${response.message()}")
                        Log.d("Keranjang", "onResponse: Membuat keranjang karena tidak punya keranjang")
                        createKeranjang(pelangganId, totalHarga, onResult, onError)
                    }
                }
        ...
        if (pelangganId != null) {
            validateOrCreateKeranjang(
                ...
                onResult = { keranjangId ->
                    // Jika validasi sukses, tambahkan detail ke keranjang
