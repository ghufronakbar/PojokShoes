...
class ActivityKeranjang : AppCompatActivity() {
    ...
    private lateinit var totalOngkirTextView: TextView
    ...
    private var currentKeranjangId: Int? = null

    private var token: String? = null

    private var isAllKeranjangAvailable:Boolean = keranjangList.all { it.layanan_status == 1 }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Log.d("isAllKeranjangAvailable", isAllKeranjangAvailable.toString())

        if(!isAllKeranjangAvailable){
            checkoutButton.alpha = 0.5f
            checkoutButton.setBackgroundColor(ContextCompat.getColor(this, R.color.red_disabled))
        }
    ...
    private fun initializeViews() {
        ...
        totalOngkirTextView = findViewById(R.id.text_ongkir)
        ...
        checkoutButton.setOnClickListener {
            if (!isAllKeranjangAvailable) {
                Toast.makeText(this, "Layanan tidak tersedia, harap hapus dari keranjang!", Toast.LENGTH_SHORT).show()
    ...
    private fun updateAdapterWithDetails() {
        ...
        val combinedList = detailKeranjangList.map { detail ->
            ItemKeranjangs(
                ...
                layanan_picture = detail.layanan_picture,
                layanan_status = detail.layanan_status
            )
        }
    ...
    private fun fetchShippingCost() {
        Log.d("Keranjang", "Fetching Shipping Cost hit")
        if(token != null){
            Log.d("Keranjang", "Token: $token")
            RetrofitClient.apiService.getShippingCost("Bearer $token")
                .enqueue(object : Callback<ShippingRespnose> {
                    override fun onResponse(call: Call<ShippingRespnose>, response: Response<ShippingRespnose>) {
                        if (response.isSuccessful) {
                            val shippingRespnose = response.body()
                            Log.d("Keranjang", "Shipping Cost: $shippingRespnose")
                            shippingRespnose?.let {
                                val currencyFormatter = NumberFormat.getCurrencyInstance(Locale("id", "ID"))
                                val shippingCost = it.shipping_cost ?: 0
                                totalOngkirTextView.text = currencyFormatter.format(shippingCost)

                                val totalPembayaran = calculateTotalPembayaran() + shippingCost
                                totalPembayaranTextView.text = currencyFormatter.format(totalPembayaran)
                                updateTotalPembayaran()
                            }
                        } else {
                            Toast.makeText(this@ActivityKeranjang, "Gagal mengambil biaya penjemputan", Toast.LENGTH_SHORT).show()
                        }
                    }

                    override fun onFailure(call: Call<ShippingRespnose>, t: Throwable) {
                        Log.e(TAG, "Error fetching shipping cost: ${t.message}")
                        Toast.makeText(this@ActivityKeranjang, "Error fetching shipping cost: ${t.message}", Toast.LENGTH_SHORT).show()
                    }
                })
        }
    }
    ...
    private fun deleteKeranjangItemFromServer(position: Int) {
        Log.d("DeleteItem", "Deleting item at position $position having detailKeranjangList size ${detailKeranjangList.size}")
        Log.d("DeleteItem", "position <= 0 || position >= detailKeranjangList.size: ${position <= 0 || position >= detailKeranjangList.size}")
    ...
    private fun updateTotalPembayaran() {
        ...
        val ongkir = totalOngkirTextView.text.toString().replace("Rp", "").trim().toIntOrNull() ?: 0
        val totalPembayaran = total + ongkir
    ...
    private fun performCheckout(keranjangId: Int) {
        val checkoutRequest : Map<String, Double> = mapOf("keranjang_id" to keranjangId.toDouble())
...
//                        null
    ...
    private fun setupMidtrans() {
        UiKitApi.Builder()
            .withMerchantClientKey("SB-Mid-client-rTMPU8glYQjPkdDQ")
            .withContext(this)
            .withMerchantUrl("https://staging.lestarikehati.com/api/webhook/midtrans/")
            .enableLog(true)
    ...
    private fun updateAvailabilityStatus() {
        isAllKeranjangAvailable = keranjangList.all { it.layanan_status == 1 }
    ...
    private fun updateCheckoutButtonState() {
        if (!isAllKeranjangAvailable) {
            ...
            checkoutButton.setBackgroundColor(ContextCompat.getColor(this, R.color.red)) // Ganti dengan warna yang sesuai
        }
    }

    private fun handleUnavailableItems() {
        val unavailableItems = keranjangList.filter { it.layanan_status == 0 }
        if (unavailableItems.isNotEmpty()) {
            // Tampilkan toast jika ada item tidak tersedia
            Toast.makeText(this, "Ada item yang tidak tersedia!", Toast.LENGTH_SHORT).show()
        }
    }
