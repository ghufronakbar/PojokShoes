...
import com.example.pojokshoesapk.databinding.ActivityFormRecolorBinding
...
class ActivityFormRecolor : AppCompatActivity(), FormPesanAdapter.OnQuantityChangeListener {
    private lateinit var binding: ActivityFormRecolorBinding
    private val TAG = "ActivityFormRecolor"
    private lateinit var sessionManager: SessionManager

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityFormRecolorBinding.inflate(layoutInflater)
    ...
    private fun setupViews() {
        binding.icbackformrecolor.setOnClickListener {
            finish()
        }
        ...
        RetrofitClient.apiService.getLayananList().enqueue(object : Callback<List<DataListLayanan>> {
            override fun onResponse(call: Call<List<DataListLayanan>>, response: Response<List<DataListLayanan>>) {
                if (response.isSuccessful) {
                    ...
                    val recolorList = layananList.filter { it.layanan_nama.contains("Recolor", ignoreCase = true) }
                    recolorList.forEach { it.quantity = 0 }
                    if (recolorList.isNotEmpty()) {
                        setupListView(recolorList)
                    } else {
                        showError("Tidak ada layanan Recolor tersedia")
                    }
                } else {
