...
import com.example.pojokshoesapk.databinding.ActivityFormFastcleanBinding
...
class ActivityFormFastClean : AppCompatActivity(), FormPesanAdapter.OnQuantityChangeListener {
    private lateinit var binding: ActivityFormFastcleanBinding
    private val TAG = "ActivityFormFastClean"
    private lateinit var sessionManager: SessionManager

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityFormFastcleanBinding.inflate(layoutInflater)
    ...
    private fun setupViews() {
        binding.icbackformfastclean.setOnClickListener {
            finish()
        }
        ...
        RetrofitClient.apiService.getLayananList().enqueue(object : Callback<List<DataListLayanan>> {
            override fun onResponse(call: Call<List<DataListLayanan>>, response: Response<List<DataListLayanan>>) {
                if (response.isSuccessful) {
                    ...
                    val fastCleanList = layananList.filter {
                        it.layanan_nama.contains("Fast Clean", ignoreCase = true)
                    }
                    fastCleanList.forEach { it.quantity = 0 }
                    if (fastCleanList.isNotEmpty()) {
                        setupListView(fastCleanList)
                    } else {
                        showError("Tidak ada layanan Fast Clean tersedia")
                    }
                } else {
